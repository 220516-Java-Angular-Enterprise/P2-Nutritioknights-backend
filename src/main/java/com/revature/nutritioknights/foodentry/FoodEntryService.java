...
public class FoodEntryService {
    ...
    public List<FoodEntryPrettyResponse>  foodPretty(GetByDateRequest request){
        ...
        for(FoodEntry entry: entryList){
            ...
            pretty.add(new FoodEntryPrettyResponse(entry.getEntry_id(),entry.getServing_amt(),food.getName(),entry.getServing_amt() * serving.getCalories().intValue(),  serving.getProtein().intValue()*entry.getServing_amt(), serving.getCarbohydrate().intValue()*entry.getServing_amt(), serving.getFat().intValue()*entry.getServing_amt()));
        }
        return pretty;
    }
